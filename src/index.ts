/* istanbul ignore file */
export { colourForIndex } from './lib/colourForIndex';
export { indicesForNormal } from './lib/indicesForNormal';
export { layerNormalForColour } from './lib/layerVertexForColour';
export { newCubeState } from './lib/factory';
export { faceForFaceOption } from './lib/face/faceForFaceOption';

export { AxisVertex } from './lib/vectors';

export { COLOURS } from './lib/colours';
export { LAYERS } from './lib/layers';
export { CubeState } from './cube/state';

export { Cube } from './cube/Cube';
export { ICube } from './cube/ICube';
export { Vector, Vertex, CubeRotationDirection } from './cube/lib';

export { FaceOption } from './lib/face';
export { IFace, FaceNormals, FacePositions, FaceColors } from './cube/IFace';

export { IOperator, Operator } from './operator';
export { Axis } from './lib/rotate';
export { Orientation } from './cube/lib';
